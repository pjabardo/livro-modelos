...
# ╔═╡ 7a286f9f-2605-464d-8e28-62994407b2a6
begin
	using Gridap
	using GridapGmsh
	using Gmsh
...
# ╔═╡ 9cc864e6-c8e4-11ed-3ef6-25d9f9723c60
md"""
# Efeito de rugosidade na transferência de calor
"""

# ╔═╡ b83a24f4-6f8e-4dbf-99bd-6c5b8bb0edca
import GeometryBasics: Point,Point2

# ╔═╡ d938e2a9-5866-4fdc-8d33-c95196dfe4d2
import PlutoUI

# ╔═╡ dcfe180b-6776-429e-afc0-9194c9f61b54
Point2(1.0,1)

# ╔═╡ d57c4190-0e3e-4b8e-94b6-b0ba5bf5118a
norm(Point(1.0, 0) - Point(0.0,0))

# ╔═╡ 1bcb686c-bd21-47f6-bcf7-2bb09e98df3c
md"""
## Definição da geometria
"""

# ╔═╡ 18165ea5-4b96-4dfc-a778-87f402095e7c
...
# ╔═╡ 4e5a1065-5262-4525-8c12-94ab695e3588
let N=4, L=1.0
	lines(koch_subdivide(N, Point(0.0, 0.0), Point(L, 0.0)))
end	

# ╔═╡ 5fb91a07-b490-4cd7-b079-c00bc4ad426f
...
# ╔═╡ 916c3014-619f-41e9-b964-a707d604d67b
numstring(x, n=2) = string(x + 10^n)[2:end]

# ╔═╡ 55b7eadf-1b72-4507-8a02-6965d4525624
md"""
## Geração da malha
"""

# ╔═╡ 340ccc17-50c2-46ee-bb3e-98a89197aa03
...
# ╔═╡ a7c213a7-85ff-414b-ba09-9f8629fa95af
...
# ╔═╡ 5b6953b0-1fe0-4796-ba17-a5bab6efcc84
md"""
| Variável |         Descrição             |      Valor     |
| -------- |:----------------------------- |:--------------:|
| N | Níveis do floco de Koch | $(@bind N PlutoUI.Slider(0:9, default=1, show_value=true))|
| Nw | Número de subdivisões na largura | $(@bind Nw PlutoUI.Slider(1:50, default=6, show_value=true)) |
| L | Comprimento do domínio | $(@bind L PlutoUI.Slider(0.1:0.1:2.0, default=1.0, show_value=true)) |
| W | Largura do domínio | $(@bind W PlutoUI.Slider(0.1:0.1:2.0, default=1.0, show_value=true)) |
"""

# ╔═╡ c4031bf4-88b4-43c8-993c-fc7b340db47b
pts = kochmeshpts(N, L, W, Nw) 

# ╔═╡ 17fc7ad0-bb17-4d02-85b3-abb264495ba9
poly(pts)

# ╔═╡ 0bb41503-32f7-4e0c-bac5-874de5dd6726
begin
	xsize = calcreflen(pts) ./ 3
	xsize[1:2] .= min(L,W) / 10
end

# ╔═╡ 1017435d-71f5-4d9e-a46e-15860175abce
basename = "heat_$(numstring(Nw,2))_$(N)"

# ╔═╡ 67b49af0-5d8a-4dd0-baaa-cd5dcf7ce066
length(pts)

# ╔═╡ 3d3457a0-dcd5-48d2-89a1-d89be0168836
fmesh = kochmesh(basename, pts, xsize)

# ╔═╡ 7c8bbb35-cb70-4679-b7e8-1d069836af03
md"""
## Elementos finitos
"""

# ╔═╡ 1a35ea36-3238-47fe-9d43-16999123fbc3
model = GmshDiscreteModel(fmesh)

# ╔═╡ 77885575-c2d1-482a-a432-205a3f528efd
Ω = Triangulation(model)

# ╔═╡ 77ec92fd-5ea5-4754-a5c1-1666c1b105ec
order = 2  # Interpolation order

# ╔═╡ a7b814b9-eb06-43b3-a2a0-629048c62543
reffe = ReferenceFE(lagrangian, Float64, order)

# ╔═╡ 35e16a8d-1877-4576-8f79-3b3f094bd9cc
dΩ = Measure(Ω, 2*order)

# ╔═╡ 9b44a7a1-ad49-4147-897c-afca738698a7
function solve_dirichlet(basename, dvals=[1.0, 0.0]; model=model, reffe=reffe, Ω=Ω, dΩ=dΩ)
...
# ╔═╡ 5e782ed8-f6ae-4d50-8c53-398e05c335fd
u = solve_dirichlet(basename)

# ╔═╡ b5a7919c-3cc3-45ef-a986-3ed6c3ae29e6
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "794cf0d40b990fee813d590618db5cc7c746f165"
...
[[deps.GLMakie]]
deps = ["ColorTypes", "Colors", "FileIO", "FixedPointNumbers", "FreeTypeAbstraction", "GLFW", "GeometryBasics", "LinearAlgebra", "Makie", "Markdown", "MeshIO", "ModernGL", "Observables", "Printf", "ShaderAbstractions", "SnoopPrecompile", "StaticArrays"]
git-tree-sha1 = "82c76c830eeaa3c499c9ec7454a871c65c43e4ef"
...
[[deps.GridapGmsh]]
deps = ["Gridap", "Libdl", "gmsh_jll"]
git-tree-sha1 = "5e3fb42c7dfab8f6f421bb13be2f76014ed9f9af"
...
git-tree-sha1 = "cfbe38cc661a5a418e198cb21cdc42258a586301"
