...
O fato de se conhecer os princípios básicos e as respectivas equações não quer dizer que o problema está resolvido. Mas é importante lembrar que estas equações continuam válidas mesmo que alguns detalhes do problema sejam nebulosos. Neste capítulo, alguns problemas (modelos) serão analisados a partir de suas equações básicas.
...
O pêndulo simples é um dos problemas mais conhecidos na física básica. A figura \ref{fig:pendulo} mostra um diagrama esquemático do pêndulo simples. A palavra ``simples'' deve ser enfatizada. Este problema introdutório em cursos de física e mecânica é uma idealização do problema real. Nesta idealização, os únicos parâmetros importantes são a massa do pêndulo, que neste modelo idealizado some, a aceleração da gravidade, que é constante, o comprimento do pêndulo e o ângulo inicial em que se solta o pêndulo. Nesta idealização, a dimensão da massa é desprezada e a única caractérística do fio é o seu comprimento que é constante. 
\subsection{O que se pode desprezar}
Mas não é tudo, outros fatores influenciam este problema como, por exemplo, resistência do ar,  geometria exata do pêndulo, elasticidade do fio, latitude, longitude a altitude do pêndulo, rotação da terra, posição do sol e outros corpos celestes, tráfego de veículos no bairro e tipo de solo onde está montado o experimento, e até mesmo variação de $\textbf{g}$  são desprezados. Se estes aspectos do problema são levados em conta, o problema é consideravelmente mais complexo. No limite, até a dinâmica dos átomos do ar e a radiação solar incidente interferem. Levando em conta tudo este problema não seria resolvido.

Neste sentido, ao se encarar o problema do ``pêndulo simples'', talvez o mais importante é saber o que ignorar! Mas para sabermos o que ignorar, é preciso conhecer pelo menos a ordem de grandeza de cada efeito listado acima na dinâmica do problema. Muitas vezes a experiência do dia a dia é suficiente para se desprezar vários dos efeitos acima. Mas uma estimativa correta destes efeitos é importante e pelo menos uma escala de grandeza destes efeitos deve ser estimada.

Imaginemos que este pêndulo é uma esfera de aço com 1 cm de diâmetro. Se esta esfera refletir toda a luz incidente, a razão entre a força gerada pela pressão da luz do sol na terra e a força da gravidade vale (ordem de grandeza):
\[
...
Este valor de 0,02\% é pequeno mas seu efeito é acumulativo e inclemente: os pêndulos param depois algum tempo! Dependendo do objetivo do estudo, este efeito deve ser incluido. Mas esta escala pode ajudar bastante. Esta força de arrasto é pequena então o modelo para a força de arrasto não precisa ser o mais sofisticado do mundo. Um coeficiente de arasto médio pode ser suficiente para a maioria das situações. Caso contrário seria necessário resolver o problema do escoamento transitório ao redor da esfera, o que, como será visto adiante, não é nada simples.
...
\]
Aqui, vale ressaltar que as condições de contorno são integrais ao problema!
...
Os parâmetros do problema $L$ e $g$ geram uma escala de tempo $t_0$.
...
\]
onde $u$ é a temperatura e $\alpha$ é o coeficiente de difusão de calor do material. Além disso, ainda faltam as condições de contorno. Quando o domínio é um cubo de lado $L$ formado pelas faces $x=(0,L)$, $y=(0,L)$ e $z=(0,L)$ e as condições de contorno forem $u=0$ na face $x=0$, $u=u_0$ e nas demais o fluxo de calor é nulo, ou seja, a derivada da temperatura em relação à normal da face é zero. A condição inicial é $u=0$. O objetivo deste problema é obter $u(x,y,z,t)$ para $t>0$:
\[
